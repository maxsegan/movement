...
To demonstrate the dataset's utility, we train MIMIC (Motion Imitation from Massive Internet Clips), a 4.0B-parameter vision-language-action model for full-body humanoid control. A vision-language backbone conditions a 24-layer diffusion transformer action head that predicts 1.6\,s (16 timesteps) of continuous joint angle trajectories. We further refine the dataset through a re-tracking and VLM-judged filtering pipeline, producing two derivative releases that we use for our final training runs: movement-strict-164 (164,390 clips that pass both programmatic continuity checks and a 235B VLM judge of skeleton-overlay quality) and movement-287 (286,890 clips that pass the VLM judge alone). On held-out validation clips, the strict-164 model achieves 39.8\textdegree{} RMSE over 22 joints across 1.5\,s of prediction (down from 43.7\textdegree{} on the original release), outperforming both static and linear extrapolation baselines, with a 5.3\% advantage on high-motion clips. The model-versus-static gap widens from 0.1\textdegree{} to 1.2\textdegree{} on average and from 1.7\textdegree{} to 3.2\textdegree{} on the high-motion subset, while training time falls from 14 days to under 6 days on the same hardware. Unlike prior humanoid VLAMs that depend on proprietary teleoperation data and large-scale compute infrastructure, our entire system (data pipeline, re-tracking, training, and inference) runs on consumer GPUs. All code, data processing pipeline, both filtered datasets, and model weights are publicly released.
\end{abstract}
...
\textbf{Composite verdict and release.} For each of the 325,036 clips in MoveNet-332, we record both the original parquet's programmatic verdict and the VLM-good verdict, replacing the tracking data with the re-tracked NPZ when the re-tracker succeeded ($v2$\_source = retrack, 193,277 clips) and falling back to the original tracking otherwise ($v2$\_source = original, 131,759 clips). This yields two distributable filtered subsets summarized in Table~\ref{tab:filtered}.

\begin{table}[h]
\centering
\caption{V2 filtered subsets of MoveNet-332. Strict requires both programmatic and VLM verdicts; vlm-good requires only the VLM judge.}
\label{tab:filtered}
\begin{tabular}{lrr}
\toprule
\textbf{Subset} & \textbf{Clips} & \textbf{\% of 325K} \\
\midrule
movenet-332 (original) & 325{,}036 & 100.0\% \\
movement-287 (vlm-good) & 286{,}890 & 88.3\% \\
movement-strict-164 (vlm-good $\land$ prog) & 164{,}390 & 50.6\% \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{MoveNet-332 dataset statistics. Strict-164 is the highest-quality filtered subset used for our primary released model.}
\label{tab:dataset}
\begin{tabular}{lrrr}
\toprule
\textbf{Metric} & \textbf{332} & \textbf{287} & \textbf{strict-164} \\
\midrule
Total clips & 331{,}656 & 286{,}890 & 164{,}390 \\
Action classes & 704 & 700 & 700 \\
Training samples & 4.68M & 3.34M & 1.86M \\
Validation samples & 95{,}774 & 174{,}197 & 59{,}677 \\
Joint angle dimensions & 22 & 22 & 22 \\
Action horizon & \multicolumn{3}{c}{16 frames (1.6s)} \\
Frame rate & \multicolumn{3}{c}{10 Hz} \\
\bottomrule
\end{tabular}
\end{table}

Each clip yields multiple non-overlapping action windows of 16 frames. The MoveNet-332 split is 98\%/2\% train/val stratified by clip-id hash. The v2 subsets use a 95\%/5\% split, since the clip-id hash distribution skews high enough that a 2\% threshold produces zero validation clips on the filtered sets. All splits prevent leakage at the clip level.
...
\textbf{Compute.} Training uses 4$\times$98GB RTX Pro Blackwell GPUs. The strict-164 model trains in 5.9 days ($\sim$566 GPU-hours), down from $\sim$14 days for the original MoveNet-332 training despite reaching comparable validation loss. The movement-287 sibling model takes 8.8 days. Adding the one-time 11-day data-refinement pass to the strict-164 training budget yields roughly 17 days end to end, all on the same workstation. For comparison, GR00T N1 reports $\sim$50K H100-hours ($\sim$\$100K at cloud rates). Peak memory is $\sim$49GB per GPU.
...
We evaluate three trained models under identical protocol: the original MIMIC model trained on the full MoveNet-332 release, and two new models trained on the filtered v2 subsets introduced in Section~\ref{sec:retracking}. Each model is evaluated on 500 randomly sampled validation clips from the matching split. Given the ground-truth pose at $t{=}0$, each method predicts joint angles for the full 16-step (1.6\,s) action horizon. We report joint-angle RMSE (degrees) on future frames ($t{=}1..15$). We compare against two baselines: \emph{static} (repeat the $t{=}0$ pose for all future frames) and \emph{linear extrapolation} (constant angular velocity from the previous frame). The high-motion subset is the top quartile by total ground-truth angular displacement ($n{=}125$).

\begin{table}[h]
\centering
\caption{Baseline comparison on 500 held-out validation clips per model. Joint-angle RMSE (degrees) on future frames ($t{=}1..15$, 1.5\,s of prediction). The original MIMIC numbers (top) match the original v1 release; the v2 numbers are from our retraining runs on the filtered subsets, evaluated on each subset's own validation split.}
\label{tab:baselines}
\begin{tabular}{lcc}
\toprule
\textbf{Method (training set, val set)} & \textbf{All} & \textbf{High Motion} \\
\midrule
Linear (332, 332-val) & 246.1\textdegree & 366.2\textdegree \\
Static (332, 332-val) & 43.8\textdegree & 65.6\textdegree \\
MIMIC v1 (332, 332-val) & 43.7\textdegree & 63.9\textdegree \\
\midrule
Linear (strict-164, strict-val) & 188.4\textdegree & 279.1\textdegree \\
Static (strict-164, strict-val) & 41.0\textdegree & 60.7\textdegree \\
\textbf{MIMIC strict-164} & \textbf{39.8\textdegree} & \textbf{57.5\textdegree} \\
\midrule
Linear (287, 287-val) & 188.8\textdegree & 288.7\textdegree \\
Static (287, 287-val) & 42.6\textdegree & 65.3\textdegree \\
\textbf{MIMIC 287} & \textbf{40.9\textdegree} & \textbf{61.7\textdegree} \\
\bottomrule
\end{tabular}
\end{table}

The strict-164 model lowers all-clip RMSE by 9\% (43.7\textdegree{} to 39.8\textdegree{}) and high-motion RMSE by 10\% (63.9\textdegree{} to 57.5\textdegree{}) compared to MIMIC v1. The gap between model and static baseline widens substantially: from 0.1\textdegree{} to 1.2\textdegree{} on all clips (a 12$\times$ larger margin) and from 1.7\textdegree{} to 3.2\textdegree{} on the high-motion subset (5.3\% reduction in error versus 2.6\% in v1). This is the strongest signal that the retracking-plus-VLM filtering meaningfully improved the training distribution: the model is not just hitting a slightly easier validation set, it is also pulling further ahead of the static reference computed on the same clips.

The movement-287 model, trained on the broader vlm-good subset that retains lower-motion activities, reaches the same long-horizon RMSE as strict-164 (40.9\textdegree{} vs 39.8\textdegree{} on each model's own val set, near-identical when evaluated on the same val set) but produces sharper short-horizon predictions: step-1 RMSE of 9.6\textdegree{} vs 11.8\textdegree{} for strict-164, and step-1 median of 2.5\textdegree{} vs 2.9\textdegree{}. We attribute this to broader coverage of low-amplitude motion in the 287 set, which sharpens local pose dynamics without changing the long-horizon ceiling.

\textbf{Per-sample distribution.} For strict-164 at step $S{=}16$, the model achieves median 28.1\textdegree{} (p25\,=\,12.4\textdegree{}, p75\,=\,45.8\textdegree{}), an improvement over v1's median of 30.4\textdegree{}.
...
We have presented MIMIC, an open-source vision-language-action model for full-body humanoid control, and MoveNet-332, a dataset of 332K processed video clips with 3D poses and instruction captions, both trained entirely from human video. We further introduce a second-pass refinement of the dataset, combining a stricter re-tracker and a 235B VLM judge of skeleton-overlay quality, that yields two filtered releases (movement-strict-164 with 164K clips and movement-287 with 287K clips) and meaningfully better validation accuracy under the same architecture and training budget. Our key findings are: (1) the GR00T N1-style dual-system architecture transfers effectively to human video data, requiring no robotic teleoperation; (2) DiT scaling yields consistent improvements up to $\sim$1.3B parameters, with diminishing returns beyond; (3) continuous angle representations (sin/cos pairs) are essential for stable training; (4) re-tracking and VLM-judged filtering reduces validation RMSE by roughly 10\% and widens the model-versus-static gap by an order of magnitude on the same architecture; and (5) the entire pipeline (data processing, re-tracking, training, and inference) runs on consumer GPUs. By releasing the complete dataset, both filtered subsets, model weights, and all training and re-tracking code, we lower the barrier to entry for humanoid VLAM research from expensive robotic infrastructure to internet-scale video processing on a single workstation. The most important next step is deployment in simulation and on physical hardware to evaluate the model in true closed-loop settings with environmental feedback.
...
\begin{itemize}
    ...
    \item \textbf{Dataset (MoveNet-332, original):} anonymized for review
...
% Dataset (332):        https://huggingface.co/datasets/maxsegan/movenet-332
