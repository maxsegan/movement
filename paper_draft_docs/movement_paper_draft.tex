...
\title{From Internet Video to Humanoid Training Data:\\An Open Curation Pipeline, Datasets, and Baseline Model}

\author{%
\begin{tabular}{c@{\hskip 4em}c}
\textbf{\large Max Segan} & \textbf{\large Hod Lipson} \\
Columbia University & Columbia University \\
{\tt\small m.segan@columbia.edu} & {\tt\small hod.lipson@columbia.edu}
\end{tabular}}
...
\begin{abstract}
We present an open pipeline that converts internet-scale human video into training data for humanoid robot control, and two curated datasets produced by it. The pipeline combines person detection, tracking, 2D-to-3D pose lifting, re-tracking with a vision-language oracle, and per-clip judgment by a 235B vision-language model, and outputs 22-DoF joint angles paired with instruction captions. Applied to Kinetics-700, it yields \textbf{movement-strict-164} (164K clips passing both programmatic and VLM verdicts) and \textbf{movement-287} (287K clips passing the VLM verdict alone). To assess whether this data supports learned control, we train MIMIC (Motion Imitation from Massive Internet Clips), a 4.0B-parameter vision-language-action baseline with a diffusion-transformer action head. We report its performance candidly: MIMIC beats linear extrapolation decisively, but exceeds a static-pose baseline by only 1.2\textdegree{} joint-angle RMSE on average (39.8\textdegree{} vs 41.0\textdegree{} over a 1.6\,s horizon), meaning most of its apparent accuracy on arbitrary clips is explained by pose persistence. Two observations are more encouraging. First, dataset curation has a measurable effect: the model-versus-static gap is an order of magnitude wider than that of an identical model trained on the unfiltered corpus. Second, errors are strongly bimodal by activity type: cyclical motions (pull-ups, squats, jumping rope) are predicted well while multi-step compositional sequences are not, suggesting the binding constraint is coverage of compositional activity rather than model capacity. The full system runs on consumer GPUs. We release the pipeline, both datasets, training code, and model weights as a foundation for future work on humanoid learning from video.
\end{abstract}
...
Vision-language-action models (VLAMs) condition motor commands on a camera image and a language instruction, unifying perception, language, and control. Recent systems (RT-2~\cite{rt2}, OpenVLA~\cite{openvla}, $\pi_0$~\cite{pi0}, GR00T N1~\cite{groot}) have shown strong generalization on robotic manipulation. Extending VLAMs to \emph{full-body humanoid control} is harder: humanoids have many more degrees of freedom than manipulators (17+ vs.\ 6--7), require whole-body coordination, and must handle dynamic balance. Open-loop systems like MotionGPT~\cite{motiongpt} and UH-1~\cite{uh1} produce motion from text without visual feedback and cannot correct for drift. Closed-loop humanoid VLAMs do exist, but Helix~\cite{helix} and GR00T N1--N1.6~\cite{groot,groot_n15,groot_n16} all rely on proprietary robotic teleoperation data and, in Helix's case, closed-source models. Without humanoid hardware or teleoperation infrastructure, researchers cannot reproduce or extend these systems.

\textbf{Our approach.} We address the data side of this problem: converting freely available human video into curated, training-ready humanoid data, and measuring honestly how far a straightforward VLAM trained on that data currently gets. Our contributions, in order of importance:
...
\begin{enumerate}
    ...
    \item \textbf{Baseline Model and Candid Evaluation:} MIMIC, a 4.0B VLAM trained on the curated data, released with an evaluation that quantifies both what it learns (a measurable, curation-dependent margin over trivial baselines; strong performance on cyclical motion) and what it does not (its average margin over a static-pose baseline is small, and compositional activities remain out of reach).
    \item \textbf{Open Infrastructure:} Complete training, re-tracking, and VLM-judging code that runs on consumer GPUs.
\end{enumerate}

We want to set expectations directly: the trained model is not close to deployable humanoid control, and we do not claim otherwise. Its value is as a measured baseline that demonstrates the datasets are trainable, isolates data curation as a variable with measurable effect, and identifies where the remaining gap lies. We consider the pipeline and datasets the primary contribution.

\begin{table*}[t]
\centering
\caption{Comparison of vision-language-action models on \emph{openness and resource profile}, not capability: the listed systems are deployed or evaluated on physical robots, while MIMIC is a video-prediction baseline (Section~\ref{sec:quantitative}). MIMIC is the only entry that is fully open-source, trains from freely available internet video, and runs on consumer hardware. Open Source column: W = weights, C = code, D = data. Training-compute figures are reported as published by each work; GPU-hours are \emph{not} directly comparable across hardware generations (A100, H100, RTX Pro Blackwell differ in throughput by 2--4$\times$).}
...
\midrule
RT-2~\cite{rt2} & 55B & Arm & 7 & \checkmark & \texttimes & Robot teleop & Not reported \\
Octo~\cite{octo} & 93M & Arm & 7 & \checkmark & W+C & Robot teleop & $\sim$8K A100-hours \\
OpenVLA~\cite{openvla} & 7B & Arm & 7 & \checkmark & W+C & Robot teleop & $\sim$21.5K A100-hours \\
$\pi_0$~\cite{pi0} & 3B & Arm & 7+ & \checkmark & \texttimes & Robot teleop & Not reported \\
UH-1~\cite{uh1} & N/R & Humanoid & 52 & \texttimes & Scripts only & Human video & Not reported \\
GR00T N1~\cite{groot} & 2.2B & Humanoid & 52 & \checkmark & Weights only & Teleop + MoCap & $\sim$50K H100-hours \\
GR00T N1.5~\cite{groot_n15} & 3.0B & Humanoid & 52 & \checkmark & Weights only & Teleop + human video & Not reported (1K H100s) \\
Helix~\cite{helix} & $\sim$7B & Humanoid & 35 & \checkmark & \texttimes & 500h teleop & Not reported \\
\midrule
\textbf{MIMIC (Ours)} & 4.0B & Humanoid & 22 & \checkmark & W+C+D & Internet video & $\sim$550 RTX Pro Blackwell-hours \\
\bottomrule
\end{tabular}%
}
\end{table*}
...
Recent extensions include $\pi_{0.5}$~\cite{pi05} (heterogeneous co-training), $\pi_0$-FAST~\cite{pi0fast} (frequency-space action tokens), Gemini Robotics 1.5~\cite{gemini15} (multi-embodiment Motion Transfer), SmolVLA~\cite{smolvla} (450M parameters with flow matching), OpenVLA-OFT~\cite{openvla_oft} (action chunking), and A2A~\cite{a2a} (single-step inference).

\subsection{Full-Body Motion from Language}

A parallel line of work generates full-body human motion from text. MotionDiffuse~\cite{motiondiffuse} and MotionGPT~\cite{motiongpt} use diffusion and GPT-style architectures respectively, but are open-loop and do not condition on visual observations. UH-1~\cite{uh1} is closest to our setup: it processes Kinetics to create the Humanoid-X dataset of 20M poses with text descriptions, but only provides scripts to recreate the dataset (not the dataset itself), uses pixel-wise motion detection rather than pose-based filtering, operates open-loop, and does not release weights.

\subsection{Learning from Human Video}

Several works learn robot control from human demonstrations: R3M~\cite{r3m} (visual representations from human video), EgoVLA~\cite{egovla} (egocentric manipulation pretraining with IK retargeting), mimic-video~\cite{mimicvideo} (internet-video models with flow-matching decoders), WholeBodyVLA~\cite{wholebodyvla} (latent actions from action-free egocentric video), and SONIC~\cite{sonic} (motion tracking scaled to 700 hours). Our work differs by extracting 3D pose directly from third-person video, requiring no egocentric data, paired demonstrations, or simulation.

\subsection{Humanoid VLAMs}

GR00T N1~\cite{groot} pairs an Eagle-2 VLM with a 16-layer DiT flow matching head (2.2B parameters total), trained on teleoperation, motion capture, and synthetic data ($\sim$50K H100-hours). NVIDIA released open weights and fine-tuning code under a non-commercial license, though pretraining data remains proprietary. GR00T N1.5~\cite{groot_n15} (3.0B) replaces Eagle-2 with an improved VLM and introduces a FLARE loss that aligns future latent representations, enabling learning from human video alongside teleoperation data. GR00T N1.6~\cite{groot_n16} (3.3B) doubles DiT capacity to 32 layers and switches to state-relative action prediction for smoother trajectories. Our architecture follows N1's dual-system design but trains entirely from internet video and runs end-to-end on a single 4-GPU workstation rather than at datacenter scale (see Section~\ref{sec:training_compute} for a quantitative comparison and its caveats).
...
We selected Kinetics-700 for scale (650K+ videos across 700 action classes), diversity, in-the-wild recording, and built-in human-action prefiltering (saving compute relative to general internet video).

Kinetics lacks 3D pose ground truth, so for pipeline validation we use NTU RGB+D 120~\cite{ntu120} (114K clips with synchronized RGB, depth, and Kinect skeleton across 120 classes). Its controlled lab conditions make it unsuitable as training data, so we use it only for accuracy validation.
...
To test whether the curated data supports learned control, we train a reference model. MIMIC's architecture follows the dual-system design introduced by Helix~\cite{helix} and adopted by GR00T N1~\cite{groot}, scaled to run on consumer hardware. We chose a conventional design deliberately: the goal is to measure what the data provides, not to innovate on architecture.
...
\textbf{Compute.} Training uses 4$\times$98GB RTX Pro Blackwell GPUs. MIMIC trains in 5.9 days ($\sim$550 GPU-hours); the 287 model takes 8.8 days. Stage 2 of the data pipeline runs once on the same hardware: 4.9 days to judge all 325K clips at 33 clips/min, 3.4 days to re-track the 217K disagreement clips at 44 clips/min, and 2.9 days to re-judge them, totalling $\sim$11 days end to end. Going from raw Kinetics-700 to a trained MIMIC fits in under 17 wall-clock days on a single workstation. For comparison, GR00T N1 reports $\sim$50K H100-hours for training alone, on different data and a different target distribution; the comparison illustrates an order-of-magnitude difference in resource requirements rather than a like-for-like efficiency gain. Peak memory is $\sim$49GB per GPU.
...
\begin{itemize}
    ...
    \item \textbf{Pipeline Angle RMSE:} Cross-system joint-angle RMSE between our pipeline and Kinect-derived ground truth (74.6\textdegree{} on NTU RGB+D 120). This figure reflects joint-mapping mismatch and Kinect body-fit error in addition to any pipeline noise, and is not a noise-floor estimate
\end{itemize}
...
Figure~\ref{fig:qualitative} shows two examples from the held-out validation set rendered as MuJoCo humanoids. Both clips are cyclical activities (rope-skipping and pull-ups), which is the regime where the model is most accurate; they should be read as favorable cases rather than typical ones. Starting from the ground-truth pose at $t{=}0$, the model's 3-step rolling predictions produce plausible trajectories that track the gross body dynamics: arm elevation and vertical compression in the rope-skipping jump cycle, and arm extension with hip drop during pull-up descent. The largest errors appear in extremities (hands, feet) where kinematic chain errors accumulate.
...
\begin{table}[h]
\centering
\caption{Baseline comparison on 500 held-out validation clips per model. Joint-angle RMSE (degrees) on future frames ($t{=}1..15$ of the 1.6\,s horizon). Each model is evaluated on its training set's own held-out split.}
\label{tab:baselines}
\begin{tabular}{lcc}
\toprule
...
\textbf{The headline result is modest, and we want to state it plainly.} MIMIC exceeds the static baseline by only 1.2\textdegree{} on average (39.8\textdegree{} vs 41.0\textdegree{}) and 3.2\textdegree{} on the high-motion subset. On a typical clip, most of the model's apparent accuracy is explained by pose persistence: human pose changes slowly at this timescale, and a baseline that simply repeats the current pose captures most of the achievable signal over 1.6\,s. Linear extrapolation fails badly (188\textdegree{}) because constant angular velocity compounds, but beating it is a low bar. A practitioner should not read Table~\ref{tab:baselines} as evidence of useful open-ended humanoid control; it is a measurement of where internet-video-only training currently stands at this scale, released so that others do not need to rediscover it.

Within that honest frame, two findings carry real information. First, \textbf{data curation has a measurable, isolated effect}: with architecture, hyperparameters, and training procedure held fixed, moving from the unfiltered 325K corpus to strict-164 widens the model-versus-static margin from 0.1\textdegree{} to 1.2\textdegree{} on all clips (12$\times$) and from 1.7\textdegree{} to 3.2\textdegree{} on high-motion clips. The model trained on unfiltered data is statistically indistinguishable from the static baseline; the model trained on curated data is not. Since the static reference is computed on the same clips in each case, this is not an artifact of an easier validation set. Second, the 287 model shows the same pattern with sharper short-horizon predictions (step-1 median 2.5\textdegree{} vs 2.9\textdegree{}), which we attribute to its broader coverage of low-amplitude motion.

\textbf{Cyclical versus multi-step activities.} The third finding explains where the model's limited skill actually lives. MIMIC's per-sample distribution at $S{=}16$ has median 28.1\textdegree{} but spreads widely (p25=12.4\textdegree{}, p75=45.8\textdegree{}), reflecting a bimodal pattern by activity type. Cyclical motions (pull-ups, squats, jumping rope, jumping jacks) cluster in the p25 tail, where the next 1.6\,s is largely periodic and recoverable from a short observation; here the model genuinely outperforms pose persistence. Multi-step compositional activities (cooking sequences, assembly tasks, sports plays with discrete phases, dance routines) cluster in the p75 tail: their next phase is not predictable from current pose alone, and the model has effectively learned nothing beyond persistence for them. Kinetics-700 has only sparse coverage of the long-horizon transitions that distinguish such activities, and DiT scaling (Section~\ref{sec:scaling}) already showed plateauing returns to parameters at this data scale, so we read the gap as a data-coverage limitation rather than a capacity ceiling. Larger video sources (Ego4D, HowTo100M, InternVid) have orders-of-magnitude more compositional material, and our pipeline can ingest them without modification. Because our evaluation setting is novel, no directly comparable published baselines exist; GR00T N1--N1.6 and Helix evaluate on physical robots with different action spaces.
...
We presented an open two-stage pipeline that turns internet human video into curated humanoid training data, two datasets produced by it (movement-strict-164 and movement-287), and MIMIC, a baseline VLAM trained on that data with a candid evaluation. The baseline's performance is limited: it exceeds a static-pose baseline by only 1.2\textdegree{} on average, and we make no claim that it constitutes useful humanoid control. What the experiments do establish: (1) the datasets are trainable, and data curation has a measurable, isolated effect, widening the model-versus-static margin by an order of magnitude under a fixed architecture and training recipe; (2) the model's skill concentrates in cyclical activities, while compositional multi-step sequences remain at pose-persistence level, pointing to coverage of long-horizon compositional activity as the binding constraint; (3) DiT scaling plateaus around 1.3B parameters at this data scale, so further architecture scaling is unlikely to close the gap on its own; and (4) the full pipeline, from raw video to trained model, runs on a single consumer workstation. We release everything (pipeline, datasets, training code, weights) in the hope that the curation machinery and the honest baseline are useful starting points for others working on humanoid learning from video. The most important next steps are ingesting video sources with denser compositional coverage and evaluating in simulation with environmental feedback.
...
\begin{itemize}
    \item \textbf{Code:} \url{https://github.com/maxsegan/movement}
    \item \textbf{Dataset (movement-strict-164):} \url{https://huggingface.co/datasets/maxsegan/movement-strict-164}
    \item \textbf{Dataset (movement-287):} \url{https://huggingface.co/datasets/maxsegan/movement-287}
    \item \textbf{Model weights (MIMIC):} \url{https://huggingface.co/maxsegan/mimic-vlam}
\end{itemize}
...
This work was supported in part by the U.S. National Science Foundation AI Institute for Dynamical Systems, grant \#2112085. We thank the authors of YOLOv8, ByteTrack, ViTPose, MotionAGFormer, and Qwen3-VL for releasing pretrained models and code, the Kinetics-700 team for the dataset, and Anthropic for Claude Code, which was used to build, run, and tune the data pipeline and training infrastructure for this work.
